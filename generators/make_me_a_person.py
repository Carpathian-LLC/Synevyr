...
countries = ["usa"]
# Realistic acquisition source weights based on B2B/SaaS industry data
referrer_weights = [
    ("organic", 0.35),      # Organic search - typically highest volume, high-quality
    ("google", 0.20),       # Google Ads - consistent performer  
    ("meta", 0.15),         # Meta/Facebook Ads - good volume but lower quality
    ("referral", 0.12),     # Direct referrals - high quality, lower volume
    ("email", 0.10),        # Email marketing - existing audience
    ("billboard", 0.05),    # Traditional advertising - brand awareness
    ("other", 0.03),        # Miscellaneous sources
]

referrers = [item[0] for item in referrer_weights]
weights = [item[1] for item in referrer_weights]
#--------------------------------------FUNCTIONS------------------------------------------

def evaluate_gender():
    # Slightly more realistic gender distribution for most B2B/consumer products
    return random.choices(["Male", "Female"], weights=[0.48, 0.52], k=1)[0]
...
def generate_person_info():
    ...
    referrer = random.choices(referrers, weights=weights, k=1)[0]
