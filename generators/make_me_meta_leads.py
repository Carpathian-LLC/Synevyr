...
# ------------------------------------------------------------------------------------
# Description:
# -> Populates `meta_leads` with a blend of real and fake lead data for ad analysis <-
#
# 30% of leads are derived from real `user_customers` to simulate captured leads from
...
def generate_meta_leads(engine, total_leads=100):
    with engine.begin() as conn:
        # Step 1: Get all users
        result = conn.execute(text("SELECT id, first_name, last_name, email FROM user_customers"))
        ...
        # Step 2: Use real users as Meta leads
        ...
        print(f"✅ Inserted {len(leads)} meta_leads.")
